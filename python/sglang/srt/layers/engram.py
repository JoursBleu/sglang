...
Predecessor tokens come from the batch itself and from a per-request history of the n - 1
tokens before each request's next token (EngramHasher.history, see there). The token map,
hash multipliers and prime layout are mirrored by the pure-torch oracle in
test/manual/deepseek_v41, which checks them against the released reference; the two copies
must stay bit-identical or the hashed ids diverge.
"""
...
def drop_checkpoint_page_cache(model_path: Optional[str] = None) -> tuple[int, int]:
    """posix_fadvise(DONTNEED) every safetensors file of the checkpoint; returns
    (files, bytes). Cached checkpoint pages are movable but compaction gives up on
    them at this scale, so a 512 MiB huge-page fault falls back to base pages while
    the cache is populated. Dropping it is what makes the private layout's
    pre-fault land on huge pages on a node that already served this checkpoint."""
...
class _HostTable:
    ...
    Layouts:
      ...
      private  one anonymous mapping per rank holding only its own rows, so the
               ...
               transparent_hugepage/enabled (madvise or always), which is on
               far more often than the shmem knob.
    """
...
class EngramEmbedding(nn.Module):
    ...
    def _dp_sharded_lookup(
        ...
        """Sharded lookup under DP attention. The TP group spans the DP ranks, each
        holding different tokens, so a rank can only serve rows for tokens it
        knows about: gather every rank's indices first, look the owned rows up
        for all of them, reassemble with the all-reduce and take back this
        rank's slice. Every rank joins, an idle one with zero rows."""
        ...
        # Same primitives as the MoE combine: a reduce-scatter when the ranks
        # hold equal (MAX_LEN) or gatherv-sized slices, else the summed buffer
        # is scattered. The reduce-scatter moves half the bytes of the
        # all-reduce, but three quarters of every row it carries are the zeros
        # of the ranks that do not own it.
