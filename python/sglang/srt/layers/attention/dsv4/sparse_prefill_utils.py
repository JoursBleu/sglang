...
def combine_topk_swa_indices(
    ...
    Args:
        topk_indices: (num_tokens, K) int32. Per-query indices into the
            ...
            owns each token. Invalid entries within ``topk_len`` must be -1;
            entries beyond ``topk_len`` are ignored.
        query_start_loc: (num_reqs+1,) int32. Cumulative query-row counts per
            request; may be in global (cross-chunk) space — kernel rebases by
            subtracting ``query_start_loc[0]``.
        query_pos: (num_tokens,) int32. Each query row's position in its
            sequence (a request's rows are consecutive, not necessarily its extend).
        ...
        out_indices: optional preallocated ``(num_tokens, combined_topk)``
            ...
            scanned-prefix length must hold across reuses).
    ...
    Returns:
        ...
        combined_lens: (num_tokens,) int32, prefix length scanned per token,
            including -1 entries that attention skips.
    """
