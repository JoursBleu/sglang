...
class DeepseekV41Compressor(nn.Module):
    ...
    def project(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if self.use_fused_gate:
            # Column views for the extend path, which still wants the halves
            # apart. They are strided rather than contiguous, which torch
            # tolerates and `pair_pool_decode` does not -- but that kernel is
            # only reachable when the fused projection is off.
...
def _small_weights_proj_max_m(n_heads: int, hidden_size: int) -> int:
    """Rows up to which the dedicated n32k5120 GEMM serves the indexer's
    `weights_proj`; -1 when the device or the shape rules it out (HIP,
    pre-sm90, a checkpoint with another head count or hidden size), so the
    caller falls through to the linear."""
...
class DeepseekV41Indexer(nn.Module):
    ...
    def __init__(
        ...
        # `weights_proj` is a [hidden -> n_heads] bf16 GEMV that cuBLAS serves in
        # two launches (split-K plus reduce, ~4-5 us at any decode batch, with an
        # sm80 cutlass pick at 2 <= m <= 8). The dedicated n32k5120 kernel is one
        # launch at 1.1-1.9 us over m <= 32 (rows split over CTAs, so the cost
        # stays flat), and it heads the chain the main stream waits for in every
        # index-source layer. Decode rows take it, everything wider keeps the
        # linear. Reduction order differs from cuBLAS by at most one bf16 ulp on
        # isolated elements (it matches tiny_gemm's).
