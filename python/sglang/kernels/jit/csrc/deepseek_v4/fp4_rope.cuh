...
/// \brief Index-K write for the DeepSeek-V4.1 low-ratio indexer, one launch.
///
/// Replaces the tail of `_low_ratio_write_group`'s index-K branch: the
/// `k_norm` RMSNorm, the RoPE tail, the fp4 fake-quant, the indexer's own fp4
/// pack and the 68-byte cache store. Only the `wk` projection stays outside.
///
/// `input` is `wk(latent)`, i.e. pre-`k_norm`. The kernel indexes `freqs_cis`
/// itself, so the caller needs neither a `freqs_cis[group_pos]` gather nor a
/// `group_pos` tensor: a ratio-`r` group's latent stands for its first
/// position, which for a power-of-two ratio is `positions & ~(r - 1)`.
struct IndexKParams {
...
/// \brief Index-Q write for the same indexer, the mirror of `IndexKParams`.
///
/// Replaces the query half of `_low_ratio_index_topk_decode`: the RoPE tail,
/// the fp4 fake-quant and the indexer's fp4 pack, leaving only the `wq_b`
/// projection outside. Four things differ from the K direction and nothing
/// else does -- no RMSNorm, so no row-spanning reduction survives at all; two
/// plain output tensors instead of the paged cache; one row per (token, head);
/// and no ratio mask, since a query rotates by its own position.
struct IndexQParams {
...
/// \brief The indexer packer's ue8m0 exponent, which is *not* `fp4::block_scale`.
///
/// `_ceil_ue8m0_exp(max(amax / 6, 1e-4))`: the floor lands after the divide
/// rather than before it, the divide is a real division rather than a multiply
/// by `1/6`, and the exponent is clamped. All three differ from the fake-quant
/// stage, so the two scales are genuinely different numbers -- which is why the
/// reference keeps both stages instead of packing the first one directly.
SGL_DEVICE uint32_t index_pack_exponent(float amax) {
  ...
  // Transcribed from the reference, but neither bound is reachable: the 1e-4
  // floor keeps the exponent well above 1, and 254 would need a block absmax
  // above `6 * 2^126`, which overflows fp32. No test can distinguish it.
...
#pragma unroll
  ...
  // Stage 2: the packer, whose scale floor differs, so it can recover an
  // exponent the fake-quant gave away. One `cvt` per pair emits the packed byte
  // directly, `.x` in the low nibble -- exactly the reference's nibble order.
  IndexPacked out;
#pragma unroll
  for (uint32_t half = 0; half < 2; ++half) {
    const auto amax = warp::reduce_max<kHalfLanes>(fmaxf(fabsf(data[half * 2]), fabsf(data[half * 2 + 1])));
...
/// \brief grid = ceil(num_tokens / kFp4RopeWarpsPerCTA), block = 32 * that.
///
/// One warp per token, and at (128, 64) the tiling falls out with no shuffles
/// and no divergence at all: a lane loads two `bf16x2` at vector indices `lane`
/// and `lane + 32`, so it owns elements `{2L, 2L+1}` of the head and
/// `{64+2L, 64+2L+1}` of the tail -- and the second pair is exactly one complex
/// pair of the RoPE tail, in the one lane that needs it. Every lane holds a
/// piece of both halves, so unlike the main-KV kernels there is no nope/rope
/// branch to split a warp.
///
/// The RMSNorm statistic is the one thing that spans the row, so it is one full
/// warp `reduce_sum` with no shared memory and no `__syncthreads`; everything
/// after it is `index_rope_quant_pack`, which the Q direction shares.
...
/// \brief grid = ceil(num_rows / kFp4RopeWarpsPerCTA), block = 32 * that.
///
/// One warp per (token, head). With no RMSNorm there is no cross-lane sum left,
/// only the four block absmaxes, so a warp is the smallest unit of work the
/// row admits and the launch is as friendly to occupancy as the shape allows.
///
/// `input` is `[num_tokens, heads, kHeadDim]` contiguous, so row `r` is head
/// `r % heads` of token `r / heads`; only the token index selects the freqs,
/// and a query rotates by its own position, so there is no ratio mask.
...
__launch_bounds__(kFp4RopeWarpsPerCTA* device::kWarpThreads) void flash_index_q_kernel(const IndexQParams params) {
  ...
  // `heads` is a runtime value, so this is a real division -- warp-uniform,
  // once, and ahead of the PDL gate, where it costs nothing the load does not.
  ...
  if constexpr (kWeights) {
    // The indexer's per-head weight, `head_weights(x).float()` in torch:
    // `weights_proj(x)` (bf16) times `softmax_scale * heads^-0.5`, computed in
    // fp32 with the scalar rounded to fp32, rounded to nearest-even bf16, then
    // widened to fp32 for the logits kernel. The two aten launches that did
    // this (a bf16 mul and a `.float()` copy) are what this replaces; the
    // arithmetic is the same so the result is bitwise.
    if (lane == 0) {
