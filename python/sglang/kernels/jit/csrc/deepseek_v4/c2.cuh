...
/// \brief grid = num_tokens, block = kHeadDim / 4.
///
...
__global__ __launch_bounds__(kHeadDim / kC2VecSize) void flash_c2_decode_kernel(const C2Params params) {
  ...
  // The ring puts the read and the write on different slots: a completing row
  // reads what `pos - 1` left, a pending one writes its own. The single row
  // this used to read-modify-write is gone.
...
#pragma unroll
  ...
  if constexpr (kStore) {
    ...
    //
    // The RoPE input is the bf16 latent just stored, not the fp32 accumulator:
    // the reference is `_rope_fq4(finish(pooled))` and `finish` rounds to bf16
    // first. Reading `out` back is how that rounding gets applied.
#pragma unroll
    for (uint32_t i = 0; i < kVecSize / 2; ++i) {
    ...
    if (tx >= kNopeThreads) {
      // Each of these threads owns kVecSize / 2 complex pairs of the tail, and
      // `rope_tail` ends in `.to(x.dtype)`, so the rotated value is rounded back
      // to bf16 before the fake-quant widens it again.
      // Only odd positions reach here, and the latent stands for `pos - 1`.
...
struct FlashC2DecodeKernel {
 ...
 private:
  ...
  static void launch(
    ...
    // All three come straight out of the caller's metadata, in the dtypes it
    // already holds them in, so no cast is needed at the call site. The
    // scheduler's `out_cache_loc` is int64; the unit tests hand int32.
