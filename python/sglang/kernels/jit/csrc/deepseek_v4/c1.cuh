...
__launch_bounds__(kHeadDim / kC1VecSize) void flash_c1_decode_kernel(const __grid_constant__ C1Params params) {
  ...
  // `out_loc` and `positions` come from the step's metadata, not from the
  // predecessor kernel, so prefetching them ahead of the PDL gate overlaps
  // with the `wkv` GEMM's tail. A padded row is dropped at the store, not
  // here: gating the loads on it would put a branch on the fast path.
...
#pragma unroll
  ...
  // before the trigger because that GEMM is the successor that reads it, and
  // published by padded rows too -- the caller drops those rows, and skipping
  // the store would only add a branch.
  latent.store(params.kv_output + row * kHeadDim, tx);
  PDLTriggerSecondary<kUsePDL>();

  // The RoPE input is the bf16 latent just stored, not the fp32 accumulator:
  // the reference is `_rope_fq4(finish(kv))` and `finish` rounds to bf16 first.
  // Reading `latent` back is how that rounding gets applied.
#pragma unroll
  for (uint32_t j = 0; j < kVecSize / 2; ++j) {
