...
def _silu_and_mul_clamp_kernel(
    ...
    u = tl.load(x_ptr + pid_m * stride_xm + I + offs, mask=mask, other=0.0).to(
        tl.float32
    )
    ...
    if EMIT_FP8:
        ...
        tl.store(
            scale_ptr + pid_m * stride_sm + goffs, e8.to(tl.uint8), mask=goffs < I // 32
        )
    else:
        if FP8_GRID:
...
def silu_and_mul_clamp_triton(
    ...
    """gate_up [M, 2 * I] -> [M, I] = silu(min(g, lim)) * clamp(u, -lim, lim).
    ...
    if fp8_grid:
        assert silu_and_mul_clamp_fp8_grid_supported(I), I
    out = torch.empty(
        (M, I),
    ...
    scale = (
        torch.empty((M, I // 32), dtype=torch.uint8, device=gate_up.device)
        if emit_fp8
        else None
    )
    ...
    if M == 0:
        return wrap()
    block_i = 1024 if I >= 1024 else triton.next_power_of_2(I)
    _silu_and_mul_clamp_kernel[(M, triton.cdiv(I, block_i))](
        gate_up,
        out,
