...
def c2_decode_norm(
    ...
    *,
    ...
    The three index arrays come straight out of the caller's metadata, in the
    dtypes it already holds them in, so this costs no per-layer tensor op: there
    is no ``state_rows`` to build, no ``c2_out_loc`` to read and no masking.
...
def c2_decode_norm_rope_store(
    ...
    *,
    ...
    The RoPE tail, the fp4 fake-quant and the 584-byte FlashMLA store all happen
    while the latent is still in registers, so the main-KV branch costs no extra
    launch and no round trip through memory. ``out`` is still produced, because
    the index-K branch's ``wk`` projection needs the pre-RoPE latent.

    The store slot is ``raw_out_loc // ratio``, derived in-kernel, so
    ``c2_out_loc`` is not read on this path at all.
